...
static struct plic_data plic = {
	.addr = RZFIVE_PLIC_ADDR,
	.num_src = RZFIVE_PLIC_NUM_SOURCES,
};
...
static int rzfive_final_init(bool cold_boot)
{
	...
	/* enable L2 cache */
	uint32_t *l2c_ctl_base = (void *)RZFIVE_L2C_ADDR + V5_L2C_CTL_OFFSET;
...
static int rzfive_console_init(void)
{
    return scif_init();
}

/* Initialize the platform interrupt controller for current HART. */
static int rzfive_irqchip_init(bool cold_boot)
{
...
static int rzfive_early_init(bool cold_boot)
{
	...
	if (cold_boot) {
		ret = plmt_cold_timer_init(RZFIVE_PLMT_ADDR,
					   RZFIVE_HART_COUNT);
		if (ret)
			return ret;
	}
